...
{
    public class FirebirdDataTypeMapper : IDataTypeMapper
    {
        public static readonly IDataTypeMapper Default = new FirebirdDataTypeMapper();
...
//GUID                |800                 |CHAR(36)            |System.Guid 
        ...
        public string ToDatabaseStoreType(DataTypeDescriptor dataType)
        {
            return dataType.DbType switch
            {
                ...
                CommonDataType.Varchar => $"VARCHAR({dataType.Arg1 ?? 1})",
                CommonDataType.Char => $"CHAR({dataType.Arg1})",
                CommonDataType.NVarchar => $"VARCHAR({dataType.Arg1 ?? 1})",
                CommonDataType.NChar => $"NCHAR({dataType.Arg1})",
                CommonDataType.Float => "FLOAT",
                CommonDataType.Double => "DOUBLE PRECISION",
                CommonDataType.Binary => $"BINARY({dataType.Arg1})",
                CommonDataType.Int => "INTEGER",
                CommonDataType.SmallInt => "SMALLINT",
                CommonDataType.BigInt => "BIGINT",
                CommonDataType.TinyInt => "SMALLINT",
                CommonDataType.Varbinary => "VARBINARY({dataType.Arg1})",
                CommonDataType.Decimal => $"DECIMAL({dataType.Arg1},{dataType.Arg2})",
                CommonDataType.Date => "DATE",
                CommonDataType.Time => "WITHOUT TIME ZONE",
                CommonDataType.DateTime => "TIMESTAMP WITHOUT TIME ZONE",
                CommonDataType.Text => "BLOB SUB_TYPE TEXT",
                CommonDataType.NText => "BLOB SUB_TYPE TEXT",
                CommonDataType.Blob => "BLOB SUB_TYPE BINARY",
                CommonDataType.Uuid => "CHAR(16) CHARACTER SET OCTETS",
                CommonDataType.DateTimeOffset => $"TIMESTAMP WITH TIME ZONE",
